(function () {
  ...
  const send = m => { if (vscode) vscode.postMessage(m); };

  /* A black panel says nothing. Surface the reason in the extension instead. */
  window.addEventListener('error', ev => {
    const where = (ev.filename || '').split('/').pop();
    send({ type: 'error', message: (ev.message || 'unknown error') + (where ? '  (' + where + ':' + ev.lineno + ')' : '') });
  });
